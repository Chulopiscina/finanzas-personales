  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {owner ? `${owner.name} · ${owner.email}` : "Resumen financiero privado"}
          </p>
        </div>
      </header>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard title="Saldo actual" value={formatCurrency(data.metrics.currentBalance)} icon={Landmark} />
        ...
        <MetricCard title="Gasto medio mensual" value={formatCurrency(data.metrics.averageMonthlyExpense)} icon={Sigma} />
        <MetricCard
          title="Mayor gasto"
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-lg border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {data.insights.map((insight) => (
              <p key={insight} className="rounded-md bg-muted px-3 py-2 text-sm text-muted-foreground">
                {insight}
              </p>
            ))}
          </div>
        ...
        <section className="rounded-lg border border-border bg-card p-5 shadow-sm">
          <h2 className="text-base font-semibold text-card-foreground">Recomendaciones</h2>
          <div className="mt-4 space-y-3">
            {data.recommendations.map((tip) => (
              <p key={tip} className="rounded-md bg-muted px-3 py-2 text-sm text-muted-foreground">
                {tip}
              </p>
            ))}
          </div>
